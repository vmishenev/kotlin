...
class PropertyLazyInitLowering(
    ...
    override fun lower(irBody: IrBody, container: IrDeclaration) {
        if (!context.propertyLazyInitialization.propertyLazyInitialization) {
            return
        }
...
private fun IrDeclaration.isCompatibleDeclaration(context: JsCommonBackendContext) =
    correspondingProperty?.let {
        it.isForLazyInit() && !it.hasAnnotation(context.propertyLazyInitialization.eagerInitializationAnnotationSymbol)
